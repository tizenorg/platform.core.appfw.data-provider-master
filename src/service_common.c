...
int send_notify(GVariant *body, char *cmd, char *service_name)
{
	GError *err = NULL;
	GList *target_list;
	...
	if (strcmp(service_name, NOTIFICATION_SERVICE) == 0)
		monitoring_app_list = notification_monitoring_list;
	else if (strcmp(service_name, BADGE_SERVICE) == 0)
		monitoring_app_list = badge_monitoring_list;
	else if (strcmp(service_name, SHORTCUT_SERVICE) == 0)
...
static void _server_register(GVariant *parameters, GDBusMethodInvocation *invocation, const gchar *sender, char *service_name)
{
	...
	if (strcmp(service_name, NOTIFICATION_SERVICE) == 0) {
		...
		if (added_list == NULL) {
			notification_monitoring_list = g_list_append(notification_monitoring_list, strdup(sender));
		...
		added_list = g_list_find_custom(badge_monitoring_list, sender,
				(GCompareFunc)_monitoring_app_list_compare_cb);
		if (added_list == NULL) {
			badge_monitoring_list = g_list_append(badge_monitoring_list, strdup(sender));
			DbgPrint("_server_register_service : register success sender is %s , length : %d",
		...
		added_list = g_list_find_custom(shortcut_monitoring_list, sender,
				(GCompareFunc)_monitoring_app_list_compare_cb);
		if (added_list == NULL) {
			shortcut_monitoring_list = g_list_append(shortcut_monitoring_list, strdup(sender));
			DbgPrint("_server_register_service : register success sender is %s , length : %d",
		...
		gpointer user_data)
{
	...
	if (g_strcmp0(method_name, "noti_service_register") == 0)
		_server_register(parameters, invocation, sender, NOTIFICATION_SERVICE);
		...
		gpointer user_data)
{
	...
	if (g_strcmp0(method_name, "badge_service_register") == 0)
		_server_register(parameters, invocation, sender, BADGE_SERVICE);
		...
		gpointer user_data)
{
	...
	if (g_strcmp0(method_name, "shortcut_service_register") == 0)
		_server_register(parameters, invocation, sender, SHORTCUT_SERVICE);
...
int _register_dbus_interface()
{
	...
	static gchar introspection_shortcut_postfix[] =
			"'>"
			"        <method name='shorcut_service_register'>"
			"          <arg type='i' name='ret' direction='out'/>"
			"        </method>"
