...
static int _add_noti(GVariant **reply_body, notification_h noti)
{
	...
	ret = send_notify(g_variant_new("(v)", body), "add_noti_notify", _monitoring_list, PROVIDER_NOTI_INTERFACE_NAME);
...
static int _update_noti(GVariant **reply_body, notification_h noti)
{
	...
	ret = send_notify(g_variant_new("(v)", body), "update_noti_notify", _monitoring_list, PROVIDER_NOTI_INTERFACE_NAME);
